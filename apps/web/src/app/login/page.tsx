...
export default function LoginPage() {
  ...
  const { user, role, loading, signInWithGoogle, sendEmailLink, tryCompleteEmailLinkSignIn } = useAuth();
  ...
  useEffect(() => {
    ...
    if (user) {
      if (role === "owner") {
        ...
        // No role yet - try claim-role page for pending invites
  ...
  useEffect(() => {
    // If the user clicked an email link, complete sign-in.
    tryCompleteEmailLinkSignIn().catch((err) => setError(err.message));
